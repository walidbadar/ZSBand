			 ...
			 const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	ret = rm69310_cmd_data(dev, MIPI_DCS_SET_COLUMN_ADDRESS, cmd, 4);
	if (ret < 0) {
		return ret;
	}
	...
	ret = rm69310_cmd_data(dev, MIPI_DCS_SET_PAGE_ADDRESS, cmd, 4);
	if (ret < 0) {
		return ret;
	}
...
static int rm69310_set_pixel_format(const struct device *dev, const enum display_pixel_format fmt)
{
	...
	ret = rm69310_cmd_data(dev, MIPI_DCS_SET_PIXEL_FORMAT, &colmod, sizeof(uint8_t));
	if (ret < 0) {
		return ret;
	}
...
static int rm69310_configure(const struct device *dev)
{
	...
	ret = rm69310_cmd_data(dev, RM69310_MCS_SET_PAGE, &cmd, sizeof(uint8_t));
	if (ret < 0) {
		return ret;
	}

	cmd = RM69310_MCS_SPI_WRAM_BIT;
	ret = rm69310_cmd_data(dev, RM69310_MCS_SPI_WRAM_CMD, &cmd, sizeof(uint8_t));
	if (ret < 0) {
		return ret;
	}
	...
	cmd = 0;
	ret = rm69310_cmd_data(dev, MIPI_DCS_SET_TEAR_ON, &cmd, sizeof(uint8_t));
	if (ret < 0) {
		return ret;
	}

	cmd = UINT8_MAX;
	ret = rm69310_cmd_data(dev, MIPI_DCS_SET_DISPLAY_BRIGHTNESS, &cmd, sizeof(uint8_t));
	if (ret < 0) {
		return ret;
	}
...
#define RM69310_WORD_SIZE(n)								           \
	((DT_INST_STRING_UPPER_TOKEN(n, mipi_mode) == MIPI_DBI_MODE_SPI_4WIRE) ?                   \
...
#define RM69310_INIT(n)                                                                            \
	...
	static const struct rm69310_config rm69310_config_##n = {                                  \
		...
		.dbi_config = MIPI_DBI_CONFIG_DT_INST(n, RM69310_WORD_SIZE(n) |                    \
